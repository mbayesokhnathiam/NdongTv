@extends('layouts.app', ['activePage' => 'abonnes-management', 'titlePage' => __('Modifier Abonné')])
...
<div class="content">
    <div class="container-fluid">
        <div class="row">
            <div class="col-md-12">
                <form method="post" action="{{ route('abonnes.store') }}" autocomplete="off" class="form-horizontal">
                    @csrf
                    @method('post')

                    <div class="card ">
                        <div class="card-header card-header-success">
                            <h4 class="card-title">{{ __('Modifier abonné') }}</h4>
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-7">
                                    <div class="form-group{{ $errors->has('nom') ? ' has-danger' : '' }}">
                                        <input type="hidden" id="abonnement_id" name="abonnement_id" value="{{ $abonnement->id }}">
                                        <input type="hidden" name="abonnes_id" value="{{ $abonnement->abonne->id }}">
                                        <input class="form-control{{ $errors->has('nom') ? ' is-invalid' : '' }}"
                                            ...
                                            aria-required="true" value="{{old('montant') ? old('montant') : $abonnement->abonne->nom}}"/>
                                        @if ($errors->has('nom'))
                                ...
                                <div class="col-sm-7">
                                    <div class="form-group{{ $errors->has('prenom') ? ' has-danger' : '' }}">
                                        <input class="form-control{{ $errors->has('prenom') ? ' is-invalid' : '' }}"
                                            ...
                                            placeholder="{{ __('Prénom ex: Lamine') }}" value="{{old('montant') ? old('montant') : $abonnement->abonne->prenom}}" required />
                                ...
                                <div class="col-sm-7">
                                    <div class="form-group{{ $errors->has('telephone') ? ' has-danger' : '' }}">
                                        <input class="form-control{{ $errors->has('telephone') ? ' is-invalid' : '' }}"
                                            ...
                                            placeholder="{{ __('Téléphone ex: 77 000 00 00') }}" value="{{old('telephone') ? old('telephone') : $abonnement->abonne->telephone}}" required />
                                ...
                                <div class="col-sm-7">
                                    <div class="form-group{{ $errors->has('nb_tv') ? ' has-danger' : '' }}">
                                        <input class="form-control{{ $errors->has('nb_tv') ? ' is-invalid' : '' }}"
                                            ...
                                            placeholder="{{ __('NB TV ex: 10') }}" value="{{old('montant') ? old('montant') : $abonnement->nb_tv}}" required />
                                        @if ($errors->has('nb_tv'))
                            ...
                            <div class="row">
                                ...
                                <div class="col-sm-7">
                                    <div class="form-group{{ $errors->has('secteur') ? ' has-danger' : '' }}">
                                        <select id="input-secteur" class="form-control" name="secteur_id">
                                            @foreach ($secteurs as $secteur)
                                            <option value="{{ $secteur->id }}" {{ old('secteur_id') == $secteur->id || $abonnement->amply->secteur_id == $secteur->id ? "selected" : "" }}>{{ $secteur->nom_secteur }}</option>
                                            @endforeach
                                        </select>
                                        @if ($errors->has('secteur'))
                                        <span id="secteur-errorn" class="error text-danger"
                                            for="input-secteur">{{ $errors->first('secteur_id') }}</span>
                                        @endif
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="col-sm-7">
                                    <div class="form-group{{ $errors->has('amplie_id') ? ' has-danger' : '' }}">
                                        <select name="amplie_id" id="input-amplie" class="form-control">
                                            ...
                                            <option value="{{ $amplie->id }}" {{ old('amplie_id') == $amplie->id || $abonnement->amplie_id == $amplie->id ? "selected" : "" }}>{{ $amplie->adresse }}</option>
                                            @endforeach --}}
                                        </select>
                                ...
                                <div class="col-sm-7">
                                    <div class="form-group{{ $errors->has('prix_tv') ? ' has-danger' : '' }}">
                                        <input class="form-control{{ $errors->has('prix_tv') ? ' is-invalid' : '' }}"
                                            ...
                                            placeholder="{{ __('Prix unitaire ex: 2500') }}" value="{{old('montant') ? old('montant') : $abonnement->prix_tv}}" required />
                                ...
                                <div class="col-sm-7">
                                    <div class="form-group{{ $errors->has('montant') ? ' has-danger' : '' }}">
                                        <input class="form-control{{ $errors->has('nb_tv') ? ' is-invalid' : '' }}"
                                            ...
                                            placeholder="{{ __('0') }}" value="{{old('montant') ? old('montant') : $abonnement->montant}}" readonly="true" />
                                ...
                                <div class="col-sm-7">
                                    <div class="form-group{{ $errors->has('reduction') ? ' has-danger' : '' }}">
                                        <input class="form-control{{ $errors->has('reduction') ? ' is-invalid' : '' }}"
                                            ...
                                            placeholder="{{ __('Réduction ex: 1000') }}" value="{{old('montant') ? old('montant') : $abonnement->reduction}}" />
                                        @if ($errors->has('nb_tv'))
